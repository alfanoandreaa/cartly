    ...
    <main className="grid min-h-screen lg:grid-cols-2">
      <section className="flex min-h-screen flex-col px-5 py-6 sm:px-10 lg:px-16">
        ...
        <div className="my-auto mx-auto w-full max-w-md py-12">
          <p className="text-sm font-semibold text-lime">{t("auth.resetEyebrow")}</p>
          <h1 className="mt-2 text-4xl font-bold tracking-tight">{t("auth.resetTitle")}</h1>
          <p className="mt-3 text-muted">{t("auth.resetSubtitle")}</p>
          <Suspense fallback={<p className="mt-8 text-sm text-muted">{t("common.loading")}</p>}>
            <ResetPasswordForm />
          </Suspense>
      ...
      <aside className="relative hidden overflow-hidden border-l border-line bg-surface lg:block">
        ...
        <blockquote className="absolute bottom-16 left-12 right-12 max-w-xl text-3xl font-medium leading-tight tracking-tight">
          {t("auth.quote2")}
          <footer className="mt-5 text-sm font-normal text-white/60">{t("auth.quoteAuthor2")}</footer>
        </blockquote>
      </aside>
    </main>
  );
